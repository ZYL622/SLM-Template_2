...
function disp(collection, num) {
  // Hide or show buttons('hint' & 'check') by 'Show Hints' & 'Show Checks'
  if (num == 1) {
...
function HideShow(in_but, in_focus) {
  // Define Buttons 'Show Hints' & 'Show Checks'
  ...
  if (in_focus == 'unsee_hint') {
    if (show_all_hint == 0) {
      ...
      show_all_hint = 1;
      console.log(disp(collection_but, 1));
    }
    else {
      // will hide hints
      ...
      show_all_hint = 0;
      console.log(disp(collection_but, 0));
      console.log(disp(collection_child , 0));
      for (let i = 1; i < 10; i++ ) {
        if (eval('hint_'+i) == 1) {
          console.log(hint_show(i));
        }
      }
    }
  }
...
function hint_show(hint_num) {
  ...
  if (hint_now == 0) {
    ...
    hint_but.style.background = "rgb(148, 208, 245)";
    hint_now = 0;
  }
...
function check_show(hint_num) {
  ...
  let hint_now = eval('check_'+hint_num)
